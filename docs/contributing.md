You can [suggest an enhancement or report an issue on GitHub](https://github.com/salasebas/altab/issues). The app's feedback command opens the same issue tracker.
...
CI is validation only. Its app bundles are not releases, the workflow does not publish or contact upstream infrastructure, and system-wide macOS interactions still require manual QA. Optional binary packaging is a separate manual workflow (`.github/workflows/release.yml`) documented in [releasing.md](releasing.md); it is not part of pull-request validation. The validation workflow pins Xcode 26.0.1, pnpm 11.5.2, ripgrep 15.2.0, SwiftFormat 0.62.1, and every reusable action. Contributors can run the same validation locally with `corepack enable`, `corepack install`, `pnpm install --frozen-lockfile`, and `scripts/validate_ci.sh` from a machine with Xcode 26, ripgrep, and SwiftFormat 0.62.1 available.
...
ShortcutRecorder is vendored under `vendor/` and consumed as a local Swift Package Manager package. Its `vendor/scripts/update_shortcut_recorder.sh` script re-fetches it from upstream, strips unused files, and regenerates its `Package.swift`. There is no remote dependency resolution at build time — `git clone && xcodebuild` is the full bootstrap.
...
* `vendor/` holds ShortcutRecorder as a local SPM package. `vendor/scripts/update_shortcut_recorder.sh` refreshes it from upstream
