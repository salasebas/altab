...
| --- | --- | --- |
| GitHub automatic “Source code (zip/tar.gz)” | Snapshot generated by GitHub for a tag or release | Convenience only; **not** the packager’s corresponding-source artifact |
| `AlTab-<release>-source.tar.gz` | Exact `git archive` from the packager at the packaged revision | **Corresponding Source** that must sit beside any binary ZIP |
