...
The importer stores `legacyPreferencesImportCompleted` only in Altab's domain. A missing source is a completed no-op, and later launches do not re-read or re-import source choices. Values already present in Altab's persistent domain always win; registered defaults do not count as explicit values.
...
| Static shortcuts | Focus, previous, cancel, close, minimize/deminimize, fullscreen, quit, hide/show app, search | Current secure shortcut dictionary containing an `NSKeyedArchiver` archive for `SRShortcut` |
| Keyboard triggers | `holdShortcut` and `nextWindowShortcut`, keyboard indices `0...8` | Current secure shortcut dictionary containing an `NSKeyedArchiver` archive for `SRShortcut` |
...
The importer recovers the six remembered choices into a value-copy of the source snapshot, then normalizes that copy through the existing source-version-gated `PreferencesMigrations` transforms with app-identity side effects disabled. Source versions are intentionally not compared with Altab's unrelated app version, which restarted at `1`; each existing migration threshold decides whether its transform applies. Only current secure shortcut dictionaries are accepted; obsolete string/data shortcut formats are skipped instead of being guessed. After normalization, allowlist validation and missing-key copying occur. Altab's normal migrations then run against Altab's own version and values.

## Regression guards

- Unit tests pin the allowlist to every currently owned key except the three bundle-identity exclusions, exercise every category and index, and verify banned keys never enter the destination.
