...
- **Row-alignment compatibility**: the restored `alignThumbnails` preference keeps its historical indices (`Leading = 0`, `Center = 1`) and adds `Trailing = 2`. Legacy physical and semantic text values normalize to those stable indices (`Left`/`Leading`, `Center`, `Right`/`Trailing`).
...
- **testRowAlignmentTextValuesBecomeStableIndexes** — legacy `Left`/`Leading`, `Center`, and `Right`/`Trailing` values normalize to indices `0`, `1`, and `2` respectively.
