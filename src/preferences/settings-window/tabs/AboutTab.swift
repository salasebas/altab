...
class AboutWindow: NSPanel {
    ...
    private func updateUsageStats() {
        let now = Date()
        let weekCount = UsageStats.count(since: now.addingTimeInterval(-7 * 24 * 3600))
        let monthCount = UsageStats.count(since: now.addingTimeInterval(-30 * 24 * 3600))
        let yearCount = UsageStats.count(since: now.addingTimeInterval(-365 * 24 * 3600))
