...
own once ⌥ is the hold) gets no OS key-repeat, so AlTab synthesizes one. The timer is armed at
...
settling the transition and can't paint AlTab's `.canJoinAllSpaces` panel until it finishes — the
