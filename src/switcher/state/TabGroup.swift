...
class TabGroup {
    ...
    /// Local replacement for the removed `TabGroupResolver.dissolution` / `GroupDissolution` API.
    /// Production tab adapter still dissolves groups here until event routing switches to the reducer (#56).
    private static func dissolution(siblingWids: [CGWindowID], leaving leavingWid: CGWindowID, presentWids: Set<CGWindowID>) -> (remainingSiblingWids: [CGWindowID], applyToWids: [CGWindowID], dissolve: Bool) {
        let remaining = siblingWids.filter { $0 != leavingWid }
        let present = remaining.filter { presentWids.contains($0) }
        return (remaining, present, present.count <= 1)
    }
    ...
    static func removedWindowFromGroup(wid: CGWindowID?, siblingWids: [CGWindowID]) {
        ...
        let d = dissolution(siblingWids: siblingWids, leaving: wid ?? 0, presentWids: presentWids)
        ...
        guard let titles = siblingTitles else {
            ...
            if !activeTab.isTabbed, !activeTab.isFullscreen,
               let oldSiblings = activeTab.tabbedSiblingWids, let activeWid = activeTab.cgWindowId {
                let presentWids = Set(Windows.list.compactMap { $0.cgWindowId })
                let d = dissolution(siblingWids: oldSiblings, leaving: activeWid, presentWids: presentWids)
