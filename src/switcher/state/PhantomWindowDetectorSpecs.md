...
- **`cgsVerdict(s, app, inVisibleList, inAllList, visibleSpaceIds, isFocused = false)`** — authoritative,
  runs ~250ms post-show off-main (`Applications.refreshIsPhantom`) with the two CGS lists. Knows both
  signals; owns the full verdict, including clearing. Disambiguation order (first match wins):
  ...
  5. `isFocused` → not a phantom (weak-signal exemption only; the focused foreground window — #5849)
  6. else → **phantom** (weak: alpha=0 / `orderOut:` on a visible Space)
