...
class Menubar {
    ...
    static private func loadPreferredIcon() {
        let image = NSImage(named: "menubar-0")!
...
class PermissionCallout: StackView {
    ...
    // So we route the click through the container; `hitTest` keeps the button visual-only, and
    // `mouseUp` runs the action and dismisses the menu. (#5771)
