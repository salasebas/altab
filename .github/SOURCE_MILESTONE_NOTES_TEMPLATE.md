...
AlTab is an independent [GPL-3.0-only](https://www.gnu.org/licenses/gpl-3.0.html) fork of [AltTab for macOS](https://github.com/lwouis/alt-tab-macos). It is **not affiliated with or endorsed by** the upstream project. Please report problems with this fork in [AlTab Issues](https://github.com/salasebas/altab/issues).
...
- App Sandbox remains disabled (as in upstream). The supported distribution channel is direct: local builds or optional Developer ID redistribution—not the Mac App Store.
...
Requirements: full **Xcode 26** selected (not Command Line Tools alone), Git, and a recent macOS that can install that Xcode. No Apple Developer account is required. Routine builds use the once-per-Mac **Local Self-Signed** identity (`scripts/codesign/setup_local.sh`); it is not a distributable identity. See the [README](https://github.com/salasebas/altab#run-altab-from-source) and [building guide](https://github.com/salasebas/altab/blob/{{TAG}}/docs/building-and-troubleshooting.md) for permissions, troubleshooting, and uninstall notes.
...
- Local builds require the once-per-Mac **Local Self-Signed** identity for stable Accessibility / Screen Recording grants across rebuilds.
- App Sandbox is disabled; Mac App Store distribution is not supported.
- Optional third-party redistribution packaging is a separate maintainer tool and is not part of this milestone's published artifacts.
